...
.DESCRIPTION
   Gets Nessus Report as a XML Object in Nessus v2 Format that can be
...
.DESCRIPTION
   Gets a vulnerability count by risk level for each host in a given Nessus
   Report
.EXAMPLE
...
.DESCRIPTION
   Allows to pull report items from a specific Nessus Report. It allows to
   filter per host and severity level(Info, Low, Medium, High and Critical).
.EXAMPLE
...
.DESCRIPTION
   Gets the audit trail for the execution of a specific Nessus Plugin against a
   host on a Report stored on a Nessus Server.
.EXAMPLE
...
.DESCRIPTION
	The Import-NessusReport cmdlet creates objects from Nessus v2 files that are 
    generated by the Nessus 4.x or 5.x scanner.
.EXAMPLE
	Return object with report configuration and general information.
	
    Import-NessusReport .\report.nessus -InfoType ReportInfo
.EXAMPLE
	Returns objects for each of the hosts scanned with Properties and Report Items
    for each.
	
    Import-NessusReport .\report.nessus
.EXAMPLE
	Looks for hosts for which a a Vulnerability was found that a Metasploit exploit
    exists and return the IP and Name of the Module.
...
.DESCRIPTION
   Import and Parses a Nessus KB file showing plugins that triggered, passed or
   information gathered. One can also provide a list of Pligin IDs to look 
   inside the given KB to know their status
.EXAMPLE
